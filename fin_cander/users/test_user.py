...
class TestRegisterView(TestCase):
	...
	def test_register_success(self):
		...
		self.assertTrue(User_get.objects.filter(username=self.user_data["username"]).exists())

	def test_register_fail(self):
		User_get.objects.create_user(self.user_credentials)

		response = self.client.post(self.register_url, self.user_data, follow=True)
		self.assertEqual(response.status_code, status.HTTP_200_OK)
		self.assertFalse(User_get.objects.filter(username=self.user_data["username"]).count() > 1)
...
class TestLoginView(TestCase):
	...
	def test_login_fail(self):
		response = self.client.post(self.login_url, data=self.wrong_credentials, follow=True)
